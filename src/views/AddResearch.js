...
const AddResearch = () => {
  ...
  // get image from pdf and call fn getContent
  const handleExtractPDF = async () => {
    ...
    fileReader.onload = async () => {
      ...
      // Set State Research
      console.log(allText);
      const jsonResearch = matchWords(allText);
      setResearch((current) => {
        return {
          ...current,
          creator: jsonResearch.creator,
          description: jsonResearch.description,
        };
      });

      // Render the first page and get the data URL of the rendered image
  ...
  const matchWords = (str) => {
    ...
    const regex_description = /บทคัดย่อ\s*(.*?)(?:\s*กิตติกรรมประกาศ|$)/s;
    const match_description = regex_description.exec(str);
    if (match_description) {
      research.description = match_description[1].trim();
    }

    // research creator
    const regex_creator = /ผู้เขียน\s*(.*?)(?:\s*อาจารย์ที่ปรึกษา|$)/s;
    const match_creator = regex_creator.exec(str);
    if (match_creator) {
      research.creator = match_creator[1].trim();
    }
  ...
  // check state if no return false
  ...
  const validateForm = () => {
    ...
    const stringLength =
      researchFiles.image.length - "data:image/png;base64,".length;
        ...
        <div className="mt-1 mb-14 md:mb-10 p-3 md:p-8 w-full bg-white shadow-md  rounded-xl">
          <form>
            ...
            <div className="flex flex-col mb-1 md:flex-row items-start">
              <div className="text-lg md:w-2/12 ">title</div>
              <input
                onChange={(e) =>
                  setResearch((current) => {
                    return {
                ...
                type="text"
                className="w-full p-2 text-sm border-solid border rounded"
                required
              />
            </div>

            <div className="flex flex-col mb-1 md:flex-row items-start">
              <div className="text-lg md:w-2/12 ">Title Alternative</div>
              <input
                onChange={(e) =>
                  setResearch((current) => {
                    return {
                      ...current,
                      title_alternative: e.target.value,
                    };
                  })
                }
                type="text"
            ...
            <div className="flex flex-col mb-1 md:flex-row items-start">
              <div className="text-lg md:w-2/12 ">Creator</div>
              <input
                onChange={(e) =>
                  setResearch((current) => {
                    return {
            ...
            <div className="flex flex-col mb-1 md:flex-row items-start">
              <div className="text-lg md:w-2/12 ">Subject</div>
              <input
                onChange={(e) =>
                  setResearch((current) => {
                    return {
            ...
            <div className="flex flex-col mb-1 md:flex-row items-start">
              <div className="text-lg md:w-2/12">Description</div>
              <textarea
                onChange={(e) => {
                  handleChange();
                  setResearch((current) => {
                ...
                id="comment"
                className="w-full p-2 text-sm border-solid border rounded min-h-fit overflow-hidden"
                required
              ></textarea>
            </div>

            <div className="flex flex-col mb-1 md:flex-row items-start">
              <div className="text-lg md:w-2/12 ">Publisher</div>
              <input
                onChange={(e) =>
                  setResearch((current) => {
                    return {
            ...
            <div className="flex flex-col mb-1 md:flex-row items-start">
              <div className="text-lg md:w-2/12 ">Contributor</div>
              <input
                onChange={(e) =>
                  setResearch((current) => {
                    return {
            ...
            <div className="flex flex-col mb-1 md:flex-row items-start">
              <div className="text-lg md:w-2/12 ">Source</div>
              <input
                onChange={(e) =>
                  setResearch((current) => {
                    return {
            ...
            <div className="flex flex-col mb-1 md:flex-row items-start">
              <div className="text-lg md:w-2/12 ">Rights</div>
              <input
                onChange={(e) =>
                  setResearch((current) => {
                    return {
            ...
            <div className="flex flex-col mt-1 mb-1 md:flex-row items-start">
              <div className="text-lg md:w-2/12 ">Document</div>
            ...
            <div className="flex justify-end items-center mt-10">
              <button
                ...
                className="px-5 py-3 text-blue-100 bg-blue-600 font-bold rounded-full"
              >
                Submit
              </button>
            </div>
